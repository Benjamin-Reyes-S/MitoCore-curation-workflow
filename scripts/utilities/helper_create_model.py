    ...
    if "setup" in scenario.keys():
        ...
        for reaction_to_set_up in reactions_to_set_up:
            ...
            if "lower_bound" in reaction_setup.keys() and "upper_bound" in reaction_setup.keys():
                # if old upper bound is lower than new lower bound, there will be an error --> assign both bounds at once
...
def get_irreversible_model(model: cobra.Model, id_addition: str, no_gpr_split: bool = False, ex_split: bool = False) -> cobra.Model:
    ...
    * id_addition: str ~ The string that is added before 'reversed' or 'forward'
    ...
    for reaction_id in model_reaction_ids:
        ...
        if reaction.lower_bound >= 0:
            continue
        # This prevents splitting of reversible reactions without GPR
        if no_gpr_split and reaction.gene_reaction_rule != "":
            continue
        # prevent exchange reactions to be split
        if ex_split and reaction.id.startswith('EX_'):
            continue
