...
def reaction_wise_sensitivity_all_splits(model: cobra.Model, objective: str, relative_step_size: float) -> Dict[str, float]:
    ...
    objective_original = solution_original # solution_original.fluxes[objective]
...
def get_kcat_candidates(
    ...
    do_all_iterations: bool = False) -> pandas.DataFrame:
    """
    Iteratively determines reactions with highest influence 
    on objective and increases their kcat until target 
    objective value is reached. Altered reactions are candidates 
    for calibration.
    
    If the sMOMENT model before kcat calibration cannot reach a 
    desired objective value it is restricted due to a set of 
    ...
    the highest sensitivity are the most restricting ones. Kcats of reactions
    above a sensitivity threshold (0.1%) are therefore increased by 10 (max 100).
    ...
    is within reach. Altered reactions are collected and candidates for 
    calibration.
    ...
    @param do_all_iterations: When true, ignores sensitivity threshold and iterate until max iterations has ben reached. Use to provide some additional flexibility for matlab optimization.
    ...
    current_objective = objective_original
    ...
    with model:
        ...
        for i in range(0, max_iterations):
            ...
            for index, reaction_sensitivity in kcat_sensitivity.iterrows():
                ...
                for split_reaction in split_reactions_in_direction:
                    ...
                    if split_reaction in scaled_enzyme_reactions_dict and i > scaled_enzyme_reactions_dict[split_reaction]['iteration']:
                        ...
                        if next_factor < 1000:
                            ...
                            print(split_reaction + ' is at 100! Skipped factor increase!')
                            continue
                ...
                current_objective = model.slim_optimize()
                ...
                if not do_all_iterations and (target_objective - current_objective < target_deviation or current_objective> target_objective):
                    print('Objective meets target objective! Stopping.')
                    break
